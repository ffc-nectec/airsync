...
fun databaseDaoModule(): DatabaseDao = JdbiDao()

fun databaseWatcher(
    filepath: String,
    tablesPattern: Map<String, List<String>>,
    lookupIsShutdown: () -> Boolean,
    onLogInput: (tableName: String, keyWhere: List<String>) -> Unit
): DatabaseWatcherDao =
    LogReader(filepath, tablesPattern = tablesPattern, onLogInput = onLogInput, lookupIsShutdown = lookupIsShutdown)
